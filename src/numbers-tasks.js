...
function getAverage(value1, value2) {
  // it's necessary because of MAX_VALUE
  const avg2 = value2 / 2;
  const avg1 = value1 / 2;
...
function parseNumberFromString(value) {
  // for numbers in scientific notation we can use toFixed()
...
function getParallelepipedDiagonal(a, b, c) {
  // d = √(a^2 + b^2 + c^2)
  // Math.hypot AssertionError -3.741657386773941 and +3.7416573867739413
...
function roundToPowerOfTen(num, pow) {
  // Math.round() rounds to the nearest integer up or down, then we get the same number of digits
...
function isPrime(n) {
  ...
  // if i > √n then n can't be a prime number
  for (let i = 2, m = Math.sqrt(n); i <= m; i += 1) {
...
function toNumber(value, def) {
  ...
  // eslint disables to use isNaN so Number.isNaN was used
...
function getIntegerPartNumber(number) {
  // it's possible to use Number.parseInt() but parseInt() works with strings too
...
function getSumOfNumbers(x1, x2, x3) {
  // we can use arguments for many values to sum them up
  return x1 + (x2 + x3);
}
